...
HelpWindow::HelpWindow( MainWindow *w, QWidget *parent, const char *name )
    : TextBrowser( parent, name ), mw( w ), shiftPressed( FALSE )
{
}
...
void HelpWindow::setSource( const QString &name )
{
    ...
    mw->setCaption( tr("Verde Documentation Reader"));
	...
	TextBrowser::setText( txt, context() );
	QUrl u( name.mid( 2 ) );
	if ( !u.ref().isEmpty() )
...
void HelpWindow::keyPressEvent( QKeyEvent *e )
{
    ...
    TextBrowser::keyPressEvent( e );
}

void HelpWindow::keyReleaseEvent( QKeyEvent *e )
{
    shiftPressed = FALSE;
    TextBrowser::keyReleaseEvent( e );
}
